...
| `SDSize.borderThin / Thick` | 1 / 1.5 | | `SDOpacity.onDarkTrack` | 0.22 (어두운 헤더 위 토글 트랙) |
...
- 햅틱은 `.sensoryFeedback(_:trigger:)`로 한다. 한 번으로 약한 **텅장방지 경고·위험·초과 알림만** `.sdHapticPattern(_:trigger:)` 연속 패턴을 쓴다 — `.warning`(경고음 + 강한 진동 2번), `.danger`(오류음 + 강한 진동 3번), 간격 0.14초. 기획서의 UIKit 용어는 아래처럼 옮긴다
...
| `notificationOccurred(.warning)` | `SDHapticPattern.warning` | 지출 저장으로 텅장방지 경고선을 넘을 때 (예산마다 1회) |
| `notificationOccurred(.error)` | `.error` / `SDHapticPattern.danger` | 금액 입력 초과 / 텅장방지 위험·초과선을 넘을 때 (예산마다 1회) |
...
| Delete (키패드 ⌫) | `.deleteBackward` | `delete.backward` |
...
| `.sdHapticPattern(_:trigger:)` | `.sdHapticPattern(.danger, trigger: count)` | 연속 햅틱 패턴 (텅장방지 알림 전용) |
...
- **텅장방지**: 상단 "텅장 방지" + ⚙ 메뉴(예산 설정 / 예산 삭제 + 확인). `SDProgressRing` 다이얼 — 링은 **남은 기간 비율**(남은 날 ÷ 시작일~만기일), 색은 사용률 단계, 가운데 "만기까지 N일" + 만기일 → 단계 배너("경고 · 예산의 75% 사용", 초과면 "초과 · 예산보다 20% 더 썼어요" + 작게 넘은 금액) → 예산 사용 현황(경고·위험 표시 막대, 지출 금액·남은 예산, 넘으면 "초과 금액") → "지출 추가"(`.sdSecondary`). 사용률은 예산 기간 지출 기준. 예산이 없으면 점선 원 + "아직 예산이 없어요" + "예산 설정하기". **만기가 지나면** "이번 예산이 끝났어요" 결과 — Pixel로 "예산보다 20% 더 썼어요 😞" / "15% 아꼈어요 👏" / "딱 맞게 썼어요 👍" + 예산·쓴 돈·차이 + "새 예산 설정하기"
- **예산 설정**: "내가 설정한 예산" + (새 예산이면) **직전 예산 결과 카드**("지난번엔 예산보다 20% 더 썼어요 😞"(`danger` 톤) 또는 "15% 아꼈어요 👏" + 작게 "1,000,000원 중 1,200,000원 · 이번엔 꼭 지켜 보아요") + 행 5개(예산 금액·시작일·만기일·위험 알림 금액·경고 알림 금액) → 알림 구간 미리보기 → 하단 "저장하기". 금액은 키패드 시트, 날짜는 캘린더 시트("이 날짜로 설정해요"). 새 예산은 오늘 ~ 이번 달 말일로 시작. 만기 지난 예산은 새 예산을 저장할 때 결과로 기록한다. 예산 금액을 바꾸면 손대지 않은 경고·위험 금액은 70·90%로 다시 맞춘다. 제약(0 < 경고 < 위험 ≤ 예산, 시작일 ≤ 만기일)을 어기면 문구를 보이고 저장을 막는다. 저장 → "예산을 저장했어요" → 0.65초 후 텅장방지로(지출 리스트에서 왔으면 텅장방지를 연다)
- **예산 결과 표시**: 끝난·초과한 예산은 금액보다 **예산 대비 비율**을 먼저 보여준다(예산 크기와 상관없이 비교되도록). (쓴 돈 − 예산) ÷ 예산을 반올림하되 조금이라도 넘거나 남았으면 0%로 표시하지 않는다. 금액은 보조로 작게
- **텅장방지 알림**: 지출을 저장해 경고·위험·초과선을 **새로 넘으면** 저장 토스트 대신 알림 토스트("경고 금액을 넘었어요. 지출에 주의해요" 등) + `.sdHapticPattern`. 이미 알린 단계는 예산마다 기록해 다시 알리지 않고, 예산을 다시 저장하면 초기화한다
...
| 예산 알림(로컬 푸시) 발송 조건 | 앱 안 알림(토스트 + 연속 햅틱)으로 결정. 로컬 푸시는 보류 |
| "0원의 기적" 카드 이름과 연결 화면(텅장방지) | 카드는 텅장방지로 연결. 카드 이름은 용어 정리 필요 |
